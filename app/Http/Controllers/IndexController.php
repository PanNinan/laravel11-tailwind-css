...
class IndexController extends Controller
{
    ...
    {
        session()->flash('message', '设置保存成功，下个计算周期生效!');
    ...
    public function index(Request $request)
    {
        ...
        $recordIds = DB::table('hack_hardware_result')
            ->select('id')
            ->whereRaw('(machine_id, hour) IN
        (SELECT machine_id, MAX(hour) AS max_hour
        FROM hack_hardware_result
        WHERE day = ?
        GROUP BY machine_id)', [$searchDate])
            ->where('day', $searchDate)
            ->get()
            ->pluck('id')
            ->toArray();
...
//            ->where('hour', $today->copy()->subHour()->format('G'))
    ...
    {
        $cacheKey = 'detail-' . $id . md5(json_encode(\request()->query()));
        $cacheData = Cache::remember($cacheKey, 120, function () use ($id) {
